...
## ADR-026 — Player queue mutations stay inside the Player runtime/store boundary

- تصمیم: عملیات مرتب‌سازی و حذف آیتم از Queue در همان لایه‌ی Player Store/Runtime پیاده‌سازی می‌شود و از تبدیل Player به orchestrator برای featureهای دیگر جلوگیری می‌شود.
- دلیل: در این فاز هدف تقویت رفتار Queue و جلوگیری از حالت‌های غیرقابل‌اعتماد است، نه گسترش مسئولیت‌های Player به سطح اپلیکیشن.
- پیامد: Queue با رفتار deterministic برای reordering و حذف آیتم فعلی پشتیبانی می‌شود و سایر featureها از طریق runtime controller همان contract عمومی را استفاده می‌کنند.
